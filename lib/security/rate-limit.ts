...
export type RateLimitPolicy = { routeKey: string; limit: number; windowMs: number; cost?: number; scope?: "client" | "global" };
export type RateLimitStore = (input: { routeKey: string; clientHash: string; windowStart: number; expiresAt: Date; cost: number }) => Promise<number>;
...
const sqlStore: RateLimitStore = async ({ routeKey, clientHash, windowStart, expiresAt, cost }) => {
  const result = await getPool().query<{ request_count: number }>(
    `INSERT INTO rate_limit_windows (route_key,client_hash,window_start,request_count,expires_at)
     VALUES ($1,$2,$3,$4,$5)
     ON CONFLICT (route_key,client_hash,window_start) DO UPDATE SET request_count=rate_limit_windows.request_count+EXCLUDED.request_count
     RETURNING request_count`,
    [routeKey, clientHash, windowStart, cost, expiresAt],
  );
...
export async function enforceRateLimit(request: Request, policy: RateLimitPolicy, store: RateLimitStore = sqlStore) {
  ...
  const cost = Math.max(1, Math.ceil(policy.cost ?? 1));
  const count = await store({ routeKey: policy.routeKey, clientHash: policy.scope === "global" ? "global" : requestFingerprint(request), windowStart, expiresAt: new Date((windowStart + 2) * policy.windowMs), cost });
